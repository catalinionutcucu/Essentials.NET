...
public static class ServiceExtensions
{
    ...
    private static void RegisterScopedServices(IServiceCollection serviceCollection, Assembly assembly)
    {
        serviceCollection.Scan(scan => scan
                                       ...
                                       // .As(type => type.GetInterfaces().Where(implementedInterface => implementedInterface != typeof(IScopedService)).ToList())
                                       // .AsSelf()
                                       .As(type => type.GetInterfaces().Where(implementedInterface => implementedInterface != typeof(IScopedService)).DefaultIfEmpty(type).ToList())
                                       .WithScopedLifetime());
    }
    ...
    private static void RegisterSingletonServices(IServiceCollection serviceCollection, Assembly assembly)
    {
        serviceCollection.Scan(scan => scan
                                       ...
                                       .UsingRegistrationStrategy(RegistrationStrategy.Append)
                                       // .As(type => type.GetInterfaces().Where(implementedInterface => implementedInterface != typeof(ISingletonService)).ToList())
                                       // .AsSelf()
                                       .As(type => type.GetInterfaces().Where(implementedInterface => implementedInterface != typeof(ISingletonService)).DefaultIfEmpty(type).ToList())
    ...
    private static void RegisterTransientServices(IServiceCollection serviceCollection, Assembly assembly)
    {
        serviceCollection.Scan(scan => scan
                                       ...
                                       .UsingRegistrationStrategy(RegistrationStrategy.Append)
                                       // .As(type => type.GetInterfaces().Where(implementedInterface => implementedInterface != typeof(ITransientService)).ToList())
                                       // .AsSelf()
                                       .As(type => type.GetInterfaces().Where(implementedInterface => implementedInterface != typeof(ITransientService)).DefaultIfEmpty(type).ToList())
